    ...
    <div v-else-if="screen" class="space-y-6">
      <div class="flex justify-between items-center">
        <div>
          ...
          <p class="text-gray-600">{{ screen.device_id }}</p>
        </div>
      ...
      <div class="grid grid-cols-1 lg:grid-cols-3 gap-6">
        ...
        <Card title="General Information">
          <dl class="space-y-3">
            <div>
              ...
              <dd class="mt-1 text-sm text-gray-900">{{ screen.device_id }}</dd>
            </div>
            <div>
              <dt class="text-sm font-medium text-gray-500">Location</dt>
              <dd class="mt-1 text-sm text-gray-900">{{ screen.location || 'N/A' }}</dd>
            </div>
            <div>
              ...
              <dd class="mt-1">
                <span
                  :class="[
                    ...
                    screen.is_online ? 'bg-green-100 text-green-800' : 'bg-red-100 text-red-800',
                  ]"
                >
            ...
            <div>
              <dt class="text-sm font-medium text-gray-500">Active Template</dt>
              <dd class="mt-1 text-sm text-gray-900">
        ...
        <Card title="Health Metrics">
          <dl class="space-y-3">
            <div>
              ...
              <dd class="mt-1">
                <div class="flex items-center">
                  ...
                  <span class="text-sm text-gray-900">{{ healthMetrics.cpu_usage || 0 }}%</span>
                </div>
              </dd>
            </div>
            <div>
              ...
              <dd class="mt-1">
                <div class="flex items-center">
                  ...
                  <span class="text-sm text-gray-900">{{ healthMetrics.memory_usage || 0 }}%</span>
                </div>
              </dd>
            </div>
            <div>
            ...
            <div>
              <dt class="text-sm font-medium text-gray-500">Brightness</dt>
              <dd class="mt-1 text-sm text-gray-900">
        ...
        <Card title="System Information">
          <dl class="space-y-3">
            <div>
              ...
              <dd class="mt-1 text-sm text-gray-900">{{ screen.app_version || 'N/A' }}</dd>
            </div>
            <div>
              <dt class="text-sm font-medium text-gray-500">OS Version</dt>
              <dd class="mt-1 text-sm text-gray-900">{{ screen.os_version || 'N/A' }}</dd>
            </div>
            <div>
              <dt class="text-sm font-medium text-gray-500">Device Model</dt>
              <dd class="mt-1 text-sm text-gray-900">{{ screen.device_model || 'N/A' }}</dd>
            </div>
            <div>
            ...
            <div>
              <dt class="text-sm font-medium text-gray-500">Last IP</dt>
              <dd class="mt-1 text-sm text-gray-900">{{ screen.last_ip || 'N/A' }}</dd>
            </div>
          </dl>
        </Card>
      </div>
      ...
      <Card title="Commands Queue">
        <div v-if="pendingCommands.length === 0" class="text-center text-gray-500 py-4">
          No pending commands
        </div>
        <Table
          v-else
        ...
        <div class="space-y-2">
          ...
          >
            <div class="flex justify-between items-start">
              <div>
                <p class="font-medium">{{ cmd.type }}</p>
                <p class="text-sm text-gray-600">{{ cmd.name }}</p>
              </div>
              <span
                :class="[
                  'px-2 py-1 rounded-full text-xs font-medium',
                  cmd.status === 'done' ? 'bg-green-100 text-green-800' : '',
                  cmd.status === 'failed' ? 'bg-red-100 text-red-800' : '',
                  cmd.status === 'pending' ? 'bg-yellow-100 text-yellow-800' : '',
                  cmd.status === 'executing' ? 'bg-blue-100 text-blue-800' : '',
                ]"
              >
                {{ cmd.status }}
              </span>
      ...
      <Card title="Recent Logs">
        <div class="space-y-2">
          <div
            ...
            class="p-3 bg-gray-50 rounded-lg"
          >
            <p class="text-sm">{{ log.message || `${log.status} - ${formatDate(log.recorded_at || log.created_at)}` }}</p>
          </div>
        </div>
      </Card>
      ...
      <Modal :show="showCommandModal" title="Send Command" @close="showCommandModal = false">
        <div class="space-y-4">
          <div>
            ...
            <select v-model="commandForm.type" required class="w-full px-3 py-2 border border-gray-300 rounded-lg">
        ...
        <template #footer>
          <button type="button" @click="handleSendCommand" class="px-4 py-2 bg-blue-600 text-white rounded-lg hover:bg-blue-700">
            Send Command
          </button>
          <button type="button" @click="showCommandModal = false" class="px-4 py-2 border border-gray-300 rounded-lg">
            Cancel
          </button>
        </template>
      </Modal>
      ...
      <Modal :show="showTemplateModal" title="Activate Template" @close="showTemplateModal = false">
        <div class="space-y-4">
          <div>
            ...
            <select v-model="templateForm.template_id" required class="w-full px-3 py-2 border border-gray-300 rounded-lg">
          ...
          <div>
            <label class="flex items-center">
              <input v-model="templateForm.sync_content" type="checkbox" class="mr-2" />
              <span class="text-sm text-gray-700">Sync content automatically</span>
            </label>
          </div>
        </div>
        <template #footer>
          <button type="button" @click="handleActivateTemplate" class="px-4 py-2 bg-purple-600 text-white rounded-lg hover:bg-purple-700">
            Activate
          </button>
          <button type="button" @click="showTemplateModal = false" class="px-4 py-2 border border-gray-300 rounded-lg">
            Cancel
          </button>
        </template>
      </Modal>
...
const handleSendCommand = async () => {
  try {
    ...
    toastStore.success('Command sent successfully')
...
const loadCommands = async () => {
  try {
    ...
    toastStore.error(errorMsg || 'Failed to load commands')
  }
}
...
const loadLogs = async () => {
  try {
    ...
    const errorMsg = error.response?.data?.detail || error.response?.data?.message || error.message
    toastStore.error(errorMsg || 'Failed to load logs')
  }
}
